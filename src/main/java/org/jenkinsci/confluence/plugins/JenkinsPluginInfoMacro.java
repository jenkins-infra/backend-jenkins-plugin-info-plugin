...
public class JenkinsPluginInfoMacro extends BaseMacro {
	 ...
	 *            the http retrieval service to use
	 */
	...
	/**
	 * the execute method takes care of formatting the output, so NO_RENDER is
	 * returned
	 */
	...
	/**
	 * includes the given file, formats it (if specified) and returns it.
	 * 
	 * @param parameters
	 * @param body
	 *            in this case null
	...
	public String execute(Map parameters, String body,
		...
		try {
			JSONObject updateCenter = jenkinsRetriever
			...
			if (plugins.containsKey(pluginId)) {
				...
				final StatsInfoParser statsParser = getStatsParser(
						renderContext, pluginId);
				...
				boolean isGithub = getString(pluginJSON, "scm").endsWith(
						"github.com"); // Default to svn
				if (sourceDir == null) {
					sourceDir = name
							+ (isGithub && !name.endsWith("-plugin") ? "-plugin"
									: "");
				}

				String releaseTimestamp = getString(pluginJSON,
						"releaseTimestamp");
				...
				{// first row
					...
					if (isGithub) {
						String prevVer = getString(pluginJSON,
								"previousVersion");
						toBeRendered.append(githubBaseUrl).append(prevVer)
								.append("...").append(name).append('-')
								.append(version)
								.append("]\n[Since Latest Release|")
								.append(githubBaseUrl).append(version)
								.append("...master]");
					} else {
						toBeRendered
								.append(fisheyeBaseUrl)
								.append(getString(pluginJSON,
										"previousTimestamp"))
				...
				{// second row
					...
					toBeRendered
							...
							.br()
							.append(getDependencies(pluginJSON, plugins))
					...
					if (pluginJSON.containsKey("developers")) {
						JSONArray devArray = (JSONArray) pluginJSON
								.get("developers");
						Iterator<?> iterator = devArray.iterator();
						while (iterator.hasNext()) {
							JSONObject developer = (JSONObject) iterator.next();
				...
				{// third row
					if (statsParser != null) {
						toBeRendered.append(" || Usage | ").image(
								statsParser.renderChartUrl(true));
						toBeRendered.append(" || Installations | ");
						final SortedMap<Date, Integer> sortedSeries = statsParser
								.getSortedSeries();

						SimpleDateFormat df = new SimpleDateFormat("yyyy-MMM");
						for (Entry<Date, Integer> serie : sortedSeries
								.entrySet()) {
							toBeRendered.append(df.format(serie.getKey()))
									.append(' ')
									.append(serie.getValue().toString())
									.append('\n');
						}
						toBeRendered
								.append("[(?)|Plugin Installation Statistics]");
						toBeRendered.append("|\n");
					}
				}
			}
			...
			return subRenderer.render("h4. Plugin Information\n"
					+ "{warning:title=Cannot Load Update Center}\n"
					+ "ParseException: error type " + e.getErrorType() + "\n"
					+ "{warning}\n", renderContext);
		}
	...
	private String getDependencies(JSONObject pluginJSON, JSONObject plugins) {
		WikiWriter depString = new WikiWriter();
		final JSONArray depArray = (JSONArray) pluginJSON.get("dependencies");
		Iterator<?> iterator = depArray.iterator();
		while (iterator.hasNext()) {
			JSONObject dependency = (JSONObject) iterator.next();
			String depName = getString(dependency, "name");
			String depVersion = getString(dependency, "version");
			boolean depOptional = (Boolean) dependency.get("optional");
			String depWikiUrl = getWikiUrl(plugins, depName);
	...
	private String getWikiUrl(JSONObject plugins, String pluginId) {
		JSONObject plugin = (JSONObject) plugins.get(pluginId);
		return plugin == null ? "" : getString(plugin, "wiki");
	}

	private StatsInfoParser getStatsParser(RenderContext renderContext,
			String pluginId) {
		try {
			String rawStats = jenkinsRetriever.retrieveStatsResponse(
					httpRetrievalService, pluginId);
